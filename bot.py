...
quotes = [
    ...
    # можно добавить все остальные цитаты
]
...
# ---------- СТАРТ И ВЫБОР ПРОДОЛЖИТЕЛЬНОСТИ ЖИЗНИ ----------
...
# ---------- CALLBACK ДЛЯ INLINE-КНОПОК 70/80/90 ----------
...
def set_life_expectancy(call):
    ...
    bot.send_message(call.message.chat.id, f"Теперь расчёт жизни будет до {years} лет")
...
def handle_message(message):
    ...
    if user_id in awaiting_birth_date_change:
        try:
            ...
            with open("life.png", "rb") as photo:
                bot.send_photo(message.chat.id, photo, caption=f"Дата рождения обновлена. Вот твоя жизнь в неделях (до {years} лет) 🕰")
        except ValueError:
    ...
    if message.text == "Изменить дату":
        ...
        bot.send_message(message.chat.id, "Выбери предполагаемую продолжительность жизни:", reply_markup=markup)
    else:
        # если прислали дату рождения впервые
        try:
